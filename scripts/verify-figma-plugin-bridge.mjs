...
assertIncludes(ui, "服务状态", "UI service connection status");
assertIncludes(ui, "重新连接", "UI manual reconnect button");
assertIncludes(ui, "项目目录名", "UI editable project directory name");
assertIncludes(ui, "页面目录名", "UI editable page directory name");
assertIncludes(ui, "type: \"read-selection-context\"", "UI requests current selection context");
assertIncludes(ui, "projectName: projectName.value", "UI sends edited project name");
assertIncludes(ui, "pageName: pageName.value", "UI sends edited page name");
assertIncludes(ui, "同步到 UXCompiler", "UI single sync button");
assertIncludes(ui, "type: \"check-health\"", "UI health message");
assertIncludes(ui, "type: \"sync-selection\"", "UI sync message");
assertNotIncludes(ui, "type: \"export-snapshot-zip\"", "UI should keep the main surface to one sync action");
assertNotIncludes(ui, "Export Snapshot ZIP", "UI should not show a separate export button");
...
assertIncludes(main, "pageName: names.pageName", "main uses editable page name");
assertIncludes(main, "figmaPageName: figma.currentPage.name", "main captures original Figma page name");
assertIncludes(main, "selectedNodeName: root.name", "main captures selected node name");
assertIncludes(main, "message.type === \"read-selection-context\"", "main selection context handler");
assertIncludes(main, "figma.on(\"selectionchange\"", "main refreshes selection context");
