...
try {
  ...
  const componentInstanceMap = JSON.parse(readFileSync(resolve(result.artifactDir, "component_instance_map.json"), "utf8"));
  const aiDecisionReport = JSON.parse(readFileSync(resolve(result.artifactDir, "ai_decision_report.json"), "utf8"));
  ...
  assert.ok(componentInstanceMap.unmappedSourceNodeIds.length > 0);
  assert.equal(aiDecisionReport.status, "not_run");
  assert.equal(normalizationReport.source.frameNodeId, "1:1");
  assert.ok(renderStrategyManifest.regions.length > 0);
