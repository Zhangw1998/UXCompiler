...
async function writeVisualDiffArtifacts(options: WriteVisualDiffOptions): Promise<ReturnType<typeof runVisualDiff>["visualDiffReport"]> {
  ...
  await writeFile(resolve(options.outDir, "repair_patch.json"), `${JSON.stringify(result.repairPatch, null, 2)}\n`, "utf8");
  await writeFile(resolve(options.outDir, "repair_iteration_log.json"), `${JSON.stringify(result.repairIterationLog, null, 2)}\n`, "utf8");
...
async function writeArtifacts(outDir: string, artifacts: PipelineArtifacts, inputPath: string): Promise<void> {
  ...
  const files: Array<[string, unknown | string]> = [
    ...
    ["final_asset_manifest.json", artifacts.reviewedAssetManifest],
    ["final_i18n_manifest.json", artifacts.reviewedI18nManifest],
